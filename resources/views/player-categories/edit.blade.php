            ...
            <form method="POST" action="{{ route('players.update', $player->id) }}" enctype="multipart/form-data">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="flex items-center space-x-6">
                            <div class="shrink-0">
                                <img id="photoPreview" class="h-24 w-24 rounded-full object-cover"
                                     src="{{ $player->playerProfile->photo ? asset('storage/'.$player->playerProfile->photo) : asset('images/default-player.png') }}"
                                     alt="Player photo">
                            </div>
                            <div class="w-full">
                                <label class="block">
                    ...
                    <div>
                        ...
                        <input type="text" id="emp_id" name="emp_id" value="{{ $player->emp_id }}"
                               class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <select id="teams" name="teams[]" multiple class="mt-1 block w-full rounded-md border-gray-300 shadow-sm select2-multiple">
                            @foreach($teams as $team)
                                <option value="{{ $team->id }}" {{ $player->teams->contains($team->id) ? 'selected' : '' }}>
                                    {{ $team->name }}
                                </option>
                            @endforeach
                        </select>
